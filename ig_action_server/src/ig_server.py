...
class IGServer(object):
    ...
    def __init__(self, name):
        self._name = name
        self._as = actionlib.SimpleActionServer(self._name, ig_action_msgs.msg.InstructionGraphAction,
        ...
        if action.operator == MOVE:
            ...
            if self._canceled.is_canceled():
                self.publish_feedback(
                    "%s:Move(%s,%s,%s,%s,%s):CANCELED" % (node, distance, angular, speed, delta_y, rotation))
                return False
            if status:
                self.publish_feedback(
                    "%s:Move(%s,%s,%s,%s,%s):SUCCESS" % (node, distance, angular, speed, delta_y, rotation))
                return True
            else:
                self.publish_feedback(
                    "%s:Move(%s,%s,%s,%s,%s): FAILED: %s" % (node, distance, angular, speed, delta_y, rotation, msg))
                return False
